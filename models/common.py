...
########################################################### Channel swapping Vmamba module ##########################################################################
...
class WTDM(nn.Module):
    def __init__(self, in_channels):
        ...
        #self.wavelet_scale = ScaleModule([1, in_channels * 2, 1, 1], init_scale=0.1)

        self.out = nn.Sequential(
            #nn.Conv2d(4*in_channels, self.out_channels, kernel_size=1, stride=1),
...
class SRGB(nn.Module):
    def __init__(
        ...
        #self.ssm = DSDM(dim=self.d_model,downsample=None)
...
class OSA(nn.Module):
    ...
    def __init__(self, hidden_dim):
        ...
        #self.CB = CB(num_feat=hidden_dim)
        
        #self.dcn_weight = nn.Parameter(torch.empty(hidden_dim, hidden_dim, 3, 3))  # (C_out,C_in,kH,kW)
        #nn.init.kaiming_uniform_(self.dcn_weight, mode='fan_out', nonlinearity='relu')
        
        #self.dcn_bias = nn.Parameter(torch.zeros(hidden_dim))
    ...
    def forward(self, x):
        ...
        #mask = offset_mask[:, 2*8:, :, :].sigmoid()  
        
        '''
        aligned_rgb = deform_conv2d(
            input=rgb_feat,
            offset=offset,
            mask=mask,
...
if __name__ == "__main__":
    ...
    x1 = WTDM(in_channels=64).cuda()(x1)
